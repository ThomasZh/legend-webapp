...
#        ./crntb-adm.sh uninstall
        echo "Stopping webapp.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting webapp.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/webapp.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup webapp.7x24hs.com service success!'
